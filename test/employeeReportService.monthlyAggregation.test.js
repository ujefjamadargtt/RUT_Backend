...
const { aggregateMonthlyRowsByServicePO } = require('../src/services/employeeReportService');

// Minimal work-log fixture — only the fields aggregateMonthlyRowsByServicePO reads.
...
test('ticket example: PO=2hrs + Parent=3hrs + Child=4hrs collapses to ONE row totalling 9hrs, with description/status kept', () => {
  ...
  const result = aggregateMonthlyRowsByServicePO(rows);
...
test('multiple days in the month: same Service PO aggregated independently per date, never merged (ticket example)', () => {
  const rows = [
    // 04-Aug: 2 + 3 + 1 = 6 hrs
...
test('empty input -> empty output', () => {
  assert.deepEqual(aggregateMonthlyRowsByServicePO([]), []);
});
